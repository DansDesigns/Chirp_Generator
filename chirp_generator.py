...
def save_arduino_h(path, audio, sample_rate=SAMPLE_RATE, var_name="chirp"):
    ...
    Provides two arrays:
      *_DATA_U8  — unsigned 8-bit (0-255, silence=128)  → use with dacWrite()
      *_DATA_S8  — signed 8-bit  (-128..127, silence=0) → use with I2S / audioop

    The signed version avoids the audioop.ratecv bug where unsigned bytes
    are misinterpreted as signed, producing scrambled output on resample.
    """
    normalised  = np.clip(audio, -1.0, 1.0)
    samples_u8  = ((normalised + 1.0) * 127.5).astype(np.uint8)
    # Signed: subtract 128 from the unsigned values, cast to int8
    samples_s8  = (samples_u8.astype(np.int16) - 128).astype(np.int8)
    n           = len(samples_u8)

    guard = var_name.upper() + "_H"
    vn    = var_name.upper()

    def hex_rows(data, cast):
        rows = []
        for i in range(0, n, 16):
            chunk = data[i:i+16]
            row   = ", ".join(f"({cast})0x{b & 0xFF:02X}" for b in chunk)
            comma = "," if i + 16 < n else ""
            rows.append(f"  {row}{comma}")
        return rows
    ...
    lines = [
        ...
        f"//   {vn}_DATA_U8  — uint8_t  0-255  silence=128  → dacWrite(DAC1, val)",
        f"//   {vn}_DATA_S8  — int8_t  -128..127 silence=0  → I2S / audioop pipelines",
        "//",
        "// ESP32 DAC example:",
        f"//   dacWrite(DAC1, pgm_read_byte(&{vn}_DATA_U8[i]));",
        "//",
        "// I2S / audioop example (avoids the unsigned-as-signed resampler bug):",
        f"//   int8_t sample = (int8_t)pgm_read_byte(&{vn}_DATA_S8[i]);",
        "",
        f"#define {vn}_SAMPLE_RATE {sample_rate}",
        f"#define {vn}_SAMPLES      {n}",
        "",
        f"// Unsigned 8-bit — for ESP32 DAC (dacWrite)",
        f"const uint8_t {vn}_DATA_U8[] PROGMEM = {{",
        *hex_rows(samples_u8, "uint8_t"),
        "};",
        "",
        f"// Signed 8-bit — for I2S, audioop, or any pipeline expecting signed PCM",
        f"const int8_t {vn}_DATA_S8[] PROGMEM = {{",
        *hex_rows(samples_s8, "int8_t"),
        "};",
        "",
        f"#endif // {guard}",
        "",
